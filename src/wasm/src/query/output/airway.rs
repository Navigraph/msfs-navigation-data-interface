...
///
/// This function requires complete airway data, so it is expected that the provided data comes from a query by route_identifier, to ensure that full airways will be present.
///
/// When querying airways by location always be sure to query all airways with route_identifiers which appear within the query area. This is icao_code can change along one airway so it should not be used to group airways.
/// There is no way to way to identify distinct airways other than iterating through them to find an end of airway flag
