...
get_communications_at_airport(airport_ident: string): Promise<AirportCommunication[]>

get_airways_at_fix(fix_ident: string, fix_icao_code: string): Promise<Airway[]>

get_waypoints_in_range(center: Coordinates, range: NauticalMiles, area?: Area): Promise<Waypoint[]>
get_airways_in_range(center: Coordinates, range: NauticalMiles, level?: AirwayLevel): Promise<Airway[]>
```
