...
Throughout the database, waypoints, navaids, runways, airports, etc., are often used as affiliated data in procedures and airways. As such, it will be useful to have a type which encompasses all of these and can be easily tested to see which type it is.
We will call this generic type: `Fix`, and it will be a union of all the datastructures it encompasses.

Every data-structure that will be part of this union should implement `BaseFix`, and should constrain the fixType field (`BaseFix` should not be exported from the package).
...
export type Fix = EnrouteFix | IlsNavaid | Runway | OtherOnes
export type EnrouteFix = Airport | NdbNavaid | VhfNavaid | Waypoint
```
...
```ts
export interface Airport extends BaseFix {
